---
layout: two-cols-header
layoutClass: gap-x-6
---

# Lexique

::left::

<TermCard term="Abstraction" definition="Simplifier la complexité en cachant les détails d'implémentation" />

<TermCard term="Attribute" translation="(Attribut, Propriété)" definition="Variable appartenant à une classe, stockant l'état d'un objet" />

<TermCard term="Class" definition="Modèle définissant la structure et le comportement d'objets" />

<TermCard term="Constructor" translation="(Constructeur)" definition="Méthode spéciale appelée lors de la création d'un objet" />

<TermCard term="Encapsulation" definition="Principe de cacher les détails internes et contrôler l'accès" />

::right::

<TermCard term="Inheritance" translation="(Héritage)" definition="Mécanisme permettant à une classe de réutiliser une autre classe" />

<TermCard term="Instance" translation="(Objet)" definition="Réalisation concrète d'une classe en mémoire" />

<TermCard term="Interface" definition="Contrat définissant des méthodes à implémenter" />

<TermCard term="Method" translation="(Méthode)" definition="Fonction appartenant à une classe, définissant un comportement" />

<TermCard term="Polymorphism" definition="Capacité à traiter différents types d'objets de manière uniforme" />

<!--
Termes techniques essentiels de la POO
Définitions claires et concises
Traductions françaises quand pertinentes
-->
